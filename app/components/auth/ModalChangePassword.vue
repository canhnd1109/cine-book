<script setup lang="ts">
import { changePasswordSchema, type IFormChangePassword } from '~/schemas/auth.schema'

const { schema } = useSchema(changePasswordSchema)
const { t } = useI18n()

const showPass = ref(false)
const showConfirmPass = ref(false)
const passwordFocused = ref(false)
const isOpen = defineModel('isOpen', { type: Boolean, default: false })
const formRef = ref()
const form = ref<IFormChangePassword>({
  oldPassword: '',
  newPassword: '',
  confirmNewPassword: ''
})
const { isLoading = false } = defineProps<{
  isLoading?: boolean
}>()
const emit = defineEmits<{
  submit: [form: IFormChangePassword]
}>()

function checkStrength(str: string) {
  const requirements = [
    { regex: /.{8,}/, text: t('auth.at-least-8-characters') },
    { regex: /\d/, text: t('auth.at-least-1-number') },
    { regex: /[a-z]/, text: t('auth.at-least-1-lowercase-letter') },
    { regex: /[A-Z]/, text: t('auth.at-least-1-uppercase-letter') },
    { regex: /[^A-Za-z0-9]/, text: t('auth.at-least-1-special-character') }
  ]

  return requirements.map(req => ({ met: req.regex.test(str), text: req.text }))
}

const strength = computed(() => checkStrength(form.value.newPassword as string))
const score = computed(() => strength.value.filter(req => req.met).length)

const color = computed(() => {
  if (score.value === 0) return 'neutral'
  if (score.value < 3) return 'error'
  if (score.value < 5) return 'warning'
  return 'success'
})

const text = computed(() => {
  if (score.value === 0) return t('auth.enter-a-password')
  if (score.value < 3) return t('auth.weak-password')
  if (score.value < 5) return t('auth.medium-password')
  return t('auth.strong-password')
})
function handleConfirmNewPasswordEnter() {
  if (form.value.confirmNewPassword && form.value.newPassword === form.value.confirmNewPassword) {
    onSubmit()
  }
}
async function onSubmit() {
  const isValid = await formRef.value?.validate()
  if (!isValid) {
    return
  }
  emit('submit', form.value)
}

const canSubmit = computed(() => {
  return (
    form.value.oldPassword &&
    form.value.newPassword &&
    form.value.confirmNewPassword &&
    form.value.newPassword === form.value.confirmNewPassword &&
    !isLoading
  )
})
</script>
<template>
  <UModal v-model:open="isOpen" :title="t('change-password')" class="w-1/3" @submit="onSubmit">
    <template #body>
      <UForm ref="formRef" :schema :state="form" class="space-y-4">
        <UFormField :label="t('auth.old-password')" name="oldPassword">
          <UInput v-model="form.oldPassword" :placeholder="t('auth.old-password')" :ui="{ base: 'h-10' }" class="w-full" />
        </UFormField>
        <div class="flex justify-between items-center w-full">
          <div class="space-y-2 w-full">
            <!-- Password -->
            <UFormField :label="t('auth.new-password')" name="newPassword" class="w-full">
              <UInput
                ref="passwordRef"
                v-model="form.newPassword"
                :placeholder="t('auth.new-password')"
                :color="color"
                :type="showPass ? 'text' : 'password'"
                class="w-full"
                :ui="{ trailing: 'pe-1', base: 'h-10' }"
                @focus="passwordFocused = true"
                @blur="passwordFocused = false"
              >
                <template #trailing>
                  <UButton
                    color="neutral"
                    variant="link"
                    size="sm"
                    :icon="showPass ? 'i-lucide-eye-off' : 'i-lucide-eye'"
                    @click="showPass = !showPass"
                  />
                </template>
              </UInput>
            </UFormField>

            <div v-if="passwordFocused && form.newPassword" class="space-y-2">
              <UProgress :color="color" :indicator="text" :model-value="score" :max="5" size="sm" />

              <p id="password-strength" class="text-sm font-medium">{{ text }}. {{ t('auth.must-contain') }}:</p>

              <ul class="space-y-1" :aria-label="t('password-requirements')">
                <li
                  v-for="(req, index) in strength"
                  :key="index"
                  class="flex items-center gap-0.5"
                  :class="req.met ? 'text-success' : 'text-muted'"
                >
                  <UIcon :name="req.met ? 'i-lucide-circle-check' : 'i-lucide-circle-x'" class="size-4 shrink-0" />
                  <span class="text-xs font-light">{{ req.text }}</span>
                </li>
              </ul>
            </div>

            <!-- Confirm Password -->
            <UFormField :label="t('auth.confirm-password')" name="confirmPassword" class="w-full">
              <UInput
                id="confirm-password"
                ref="confirmPasswordRef"
                v-model="form.confirmNewPassword"
                :placeholder="t('auth.confirm-password')"
                :type="showConfirmPass ? 'text' : 'password'"
                class="w-full"
                :ui="{ trailing: 'pe-1', base: 'h-10' }"
                @keyup.enter="handleConfirmNewPasswordEnter"
              >
                <template #trailing>
                  <UButton
                    color="neutral"
                    variant="link"
                    size="sm"
                    :icon="showConfirmPass ? 'i-lucide-eye-off' : 'i-lucide-eye'"
                    :aria-label="showConfirmPass ? 'Hide password' : 'Show password'"
                    :aria-pressed="showConfirmPass"
                    aria-controls="confirm-password"
                    @click="showConfirmPass = !showConfirmPass"
                  />
                </template>
              </UInput>
            </UFormField>
          </div>
        </div>
      </UForm>
      <BaseButton
        :text="t('change-password')"
        variant="solid"
        class-name="w-full mt-6 flex justify-center"
        :disabled="!canSubmit"
        :is-loading="isLoading"
        @click="onSubmit"
      />
    </template>
  </UModal>
</template>

<style scoped></style>
